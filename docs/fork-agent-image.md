...
The protected publish job first verifies the successful `CI` run for the exact pushed `main` SHA, then builds, runtime-checks, SBOMs, and scans its own trusted `linux/amd64` candidate before it receives registry credentials. It pushes that image once under a unique `candidate-<run-id>-<run-attempt>` tag, captures the digest printed by that push (never by resolving a mutable tag), verifies the remote OCI manifest config digest against the locally scanned Docker image ID, and uses an immutable `repository@sha256:...` reference for metadata-only SHA-tag promotion. GitHub-signed build-provenance and package-SBOM attestations are emitted for that resulting digest. Multi-architecture publication is intentionally deferred until it can preserve this same build-once promotion guarantee.
