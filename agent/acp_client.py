...
# Probe verdicts are cached per binary path. Only definitive verdicts are
# cached; an unavailable or broken binary is retried so an in-session install
# is picked up. This protects every registered ACP adapter from a long wait
# after an unsupported transport flag exits immediately.
_ACP_PROBE_CACHE: dict[str, bool] = {}


def _acp_supported(command: str, args: list[str]) -> bool | None:
    """Tri-state check whether *command* accepts the requested ACP args."""
    if "--acp" not in args:
        return True
