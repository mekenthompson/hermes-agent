...
Only registered agents are executable. Environment variables customize a
registered entry; they never authorize an unknown ``acp://`` agent.

Resolution order for a registered agent name:
  ...
  3. The registered entry's command and arguments.
...
def is_acp_agent_available(agent_name: str) -> bool:
    """True when *agent_name* has a registered launch contract."""
    name = normalize_agent_name(agent_name)
    if not name:
        return False
    _discover_agents()
...
def resolve_agent_launch(agent_name: str) -> tuple[str, list[str]]:
    ...
    if entry is None:
        raise ValueError(
            ...
            "model-provider plugin before setting launch overrides."
        )
