...
class CopilotACPClient:
    ...
    def _session(
        self, timeout_seconds: float, *, allow_file_requests: bool = True, publish=None
        ...
        session_deadline = min(self._deadline, time.monotonic() + timeout_seconds)
        ...
        def _request(method: str, params: dict[str, Any], *, text_parts: list[str] | None = None,
            ...
            while time.monotonic() < deadline and proc.poll() is None and not self._cancelled.is_set():
                ...
                if self._handle_server_message(
                    ...
                    publish=publish_update if publish else None,
    ...
    def _run_prompt(self, prompt_text: str, *, timeout_seconds: float, model: str | None = None, publish=None) -> tuple[str, str]:
        ...
        with self._session(timeout_seconds, publish=publish) as (session, _request):
            session_id = str(session.get("sessionId") or "").strip()
    ...
    def _handle_server_message(
        ...
        allow_file_requests: bool = True, publish=None,
    ) -> bool:
