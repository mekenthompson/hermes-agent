...
slack:
  ...
  # With thread_require_mention enabled, retain automatic follow-ups only
  # in listed channels after this bot has been mentioned, has an active
  # session, or authored the thread root. This keeps separate agents from
  # joining each other's threads. Comma-separated IDs or a list.
...
| `thread_participation_channels` | With `thread_require_mention`, which channels retain follow-ups in threads this bot already participates in? | none | Listed channel threads |
