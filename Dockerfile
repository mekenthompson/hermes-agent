...
RUN apt-get -o Acquire::Retries=3 update && \
    ...
    apt-get -o Acquire::Retries=3 install -y --no-install-recommends --only-upgrade \
        libglib2.0-0t64 libmbedcrypto16 perl perl-base libperl5.40 perl-modules-5.40 && \
    rm -rf /var/lib/apt/lists/*
...
# checkout) and /opt/data (the mutable HERMES_HOME volume). Its `revision` is
# read from the install stamp; the fallback stamp's all-zero commit maps to null.
...
RUN set -eu; \
    ...
    mkdir -p /etc/hermes; \
    HERMES_IMAGE_IDENTITY="${HERMES_IMAGE_IDENTITY}" python3 -c 'import json, os, pathlib, tomllib; project = tomllib.loads(pathlib.Path("/opt/hermes/pyproject.toml").read_text(encoding="utf-8"))["project"]; stamp = json.loads(pathlib.Path("/opt/hermes/install-stamp.json").read_text(encoding="utf-8")); commit = stamp.get("commit"); revision = commit if commit and set(commit) != {"0"} else None; marker = pathlib.Path("/etc/hermes/image-provenance.json"); marker.write_text(json.dumps({"schema": 1, "deployment_kind": "image", "manager": "docker", "image": os.environ["HERMES_IMAGE_IDENTITY"], "version": project["version"], "revision": revision}, sort_keys=True, separators=(",", ":")) + "\n", encoding="utf-8"); marker.chmod(0o444)'
