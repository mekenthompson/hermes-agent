...
def _timeout_after_entry(monkeypatch, started, *, wait_started=None):
    ...
    def after_entry(future, fence, **_kwargs):
        ...
        return wait(
            future, fence, idle=0.1, ceiling=1.0,
            wait_started=cc.time.monotonic() if wait_started is None else wait_started,
        )
...
def test_host_timeout_releases_pool_slot_while_protected_provider_is_still_blocked(
    ...
    futures = _timeout_after_entry(monkeypatch, provider_started, wait_started=0.0)
